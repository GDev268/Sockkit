...
impl UdpServer {
    pub async fn new(addr: SocketAddr, config: Config) -> UdpServer {
        ...
        let thread_handle = std::thread::spawn( move || {
            Self::listening_loop(Server::bind(addr,config).unwrap(), client_tx, disconnected_clone,handle)
        });
        ...
        while !disconnected.load(Ordering::SeqCst) {
            ...
            for event in server.step() {
                ...
                match event {
                    uflow::server::Event::Connect(addr) => {
                        let (event_tx, stream) = Self::create_new_client(addr, packet_tx.clone(),handle.clone());
            ...
            while let Ok((addr,packet)) = packet_rx.try_recv() {
                ...
                if let Some(client) = server.client(&addr) {
                    client.borrow_mut().send(packet.payload.to_vec().into_boxed_slice(), packet.channel_id, packet.send_mode);
                }
            }
